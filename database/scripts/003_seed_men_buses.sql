-- =============================================
-- TUMS: Seed MEN Department Buses
-- Target: [TumsDb].[dbo].[Buses]
...
BEGIN TRY
    PRINT 'Starting MEN Department buses seed...';

    -- Create staging table for drivers/buses data
    ...
    -- Insert all driver/bus data from MEN department
    ...
    DECLARE @StagingCount INT = (SELECT COUNT(*) FROM @DriversData);
    PRINT 'Staging data prepared: ' + CAST(@StagingCount AS NVARCHAR(10)) + ' records';

    -- =============================================
    -- 1. Insert Routes first (needed for FK)
    -- =============================================
    INSERT INTO [dbo].[Routes] (RouteId, RouteName, RouteDescription, IsActive, CreatedAt, IsDeleted)
    SELECT DISTINCT
        NEWID(),
        dd.RouteName,
        dd.RouteName,
    ...
    FROM @DriversData dd
    WHERE NOT EXISTS (
        SELECT 1 FROM [dbo].[Routes] r 
        WHERE r.RouteName = dd.RouteName AND r.IsDeleted = 0
    );
    ...
    -- =============================================
    -- 2. Insert Buses with driver info
    -- Map: BusNumber, DriverName, DriverPhoneNumber, Capacity, RouteId
    -- =============================================
    -- Use CTE to get unique buses by PlateNumber+PeriodId (the unique constraint)
    ;WITH UniqueBuses AS (
        SELECT 
            dd.BusNumber,
            dd.PlateNumber,
            ...
            CASE dd.Shift 
                WHEN N'العصر' THEN 1
                WHEN N'المغرب' THEN 2
                WHEN N'الضحى' THEN 3
                ELSE 1
            END AS PeriodId,
            ROW_NUMBER() OVER (PARTITION BY dd.PlateNumber, 
                CASE dd.Shift 
                    WHEN N'العصر' THEN 1
                    WHEN N'المغرب' THEN 2
                    WHEN N'الضحى' THEN 3
                    ELSE 1
                END ORDER BY dd.BusNumber) AS rn
        FROM @DriversData dd
    )
    ...
    SELECT
        ...
        0   -- IsDeleted
    FROM UniqueBuses ub
    LEFT JOIN [dbo].[Routes] r ON r.RouteName = ub.RouteName AND r.IsDeleted = 0
    WHERE ub.rn = 1
    AND NOT EXISTS (
        SELECT 1 FROM [dbo].[Buses] b 
        WHERE b.PlateNumber = ub.PlateNumber AND b.PeriodId = ub.PeriodId AND b.IsDeleted = 0
    );
    ...
    -- Verification
    -- =============================================
    DECLARE @TotalBuses INT = (SELECT COUNT(*) FROM [dbo].[Buses] WHERE IsDeleted = 0);
    DECLARE @TotalRoutes INT = (SELECT COUNT(*) FROM [dbo].[Routes] WHERE IsDeleted = 0);

    IF @TotalBuses = 0
    BEGIN
        RAISERROR('FAIL: No buses in database after seeding!', 16, 1);
    END

    PRINT '========================================';
    PRINT 'Seed completed successfully!';
    PRINT 'Total Active Buses: ' + CAST(@TotalBuses AS NVARCHAR(10));
    PRINT 'Total Active Routes: ' + CAST(@TotalRoutes AS NVARCHAR(10));
    PRINT '========================================';
